...
# converting stickleback
gill_counts_stickleback <- as.data.frame(rna.integrated@assays$RNA@counts)
gill_bulk_stickleback <- as.data.frame(apply(gill_counts_stickleback, 1, sum))
gill_bulk_stickleback$gene <- rownames(gill_bulk_stickleback)
...
common_ids <- intersect(rownames(gill_bulk_stickleback), rownames(gill_bulk_zebrafish))
gill_bulk_stickleback <- subset(gill_bulk_stickleback, gene %in% common_ids)
gill_bulk_zebrafish <- subset(gill_bulk_zebrafish, gene %in% common_ids)

# combining in a single table for the scatterplot
gill_bulk_both <- merge(gill_bulk_stickleback, gill_bulk_zebrafish, by = "gene")
colnames(gill_bulk_both) <- c("gene", "stickleback_counts", "zebrafish_counts")
```

```{r scatter_plot_comparing_stickleback_and_zebrafish_pseudobulk}
require(ggpubr)
