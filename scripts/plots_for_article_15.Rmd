...
```{r pairwise_histogram_with_downsampling_before_PCA, fig.width=11}
load(file = "data/rna.integrated.filtered.after.clustering.obj.RData")
rna.integrated.cluster.blood <- subset(rna.integrated, idents = 0)
...
p <- ggplot(small_frame_for_hist, aes(x=dist, fill=pair, color=pair)) +
  ...
  ylab("Number cell pairs")
p
svg(filename = "plots/for_article/Suppl_histogram_of_distance_btw_cells_within_saline_and_freshwater_only_blood_equalized_heights_WITH_PROPER_DOWNSAMPLING_BEFORE_PCA.svg", width = 11, height = 6)
print(p)
dev.off()
```


```{r means_table_blood_cluster_btw_phenotype_variability_PCA_after_downsampling}
mu_btw
```
