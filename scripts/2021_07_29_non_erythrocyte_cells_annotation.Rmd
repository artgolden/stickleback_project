...
```{r}
svg(filename = "plots/Suppl_scRNA_umap_rest_very_high_res_3.svg")
...
pdf(file = "plots/scRNA_rest_avg_heatmap_markers_many_9_clusters.pdf", height = 21)
heatmap_rest_avg
dev.off()
```
...
pdf(file = "plots/scRNA_non_erythrocytes_heatmap_markers_9_clusters_percentage_of_expressing_cells_in_a_cluster_MORE_MARKERS.pdf", height = 42)
heatmap_rest_avg
dev.off()
...
pdf(file = "plots/scRNA_non_erythrocytes_heatmap_markers_9_clusters_percentage_of_expressing_cells_in_a_cluster_MORE_MARKERS_with_numbers.pdf", height = 42)
heatmap_rest_avg_with_num
dev.off()
```
...
VlnPlot(object = integrated_rest_extreme_res,
  features = c("coro1a", "arpc1b", "zgc:64051", "ptprc", "lck", "mpeg1.2", "ENSGACG00000001123"), 
  assay = "RNA", 
  same.y.lims = TRUE
  )
```
